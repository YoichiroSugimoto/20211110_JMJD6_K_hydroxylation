...
save(
    all.dt[sample(1:nrow(all.dt), size = round(nrow(all.dt) * 0.05))],
...
plotHeatmap <- function(k.count.val, JMJD6.status, plot.score.dt){
    hm <- Heatmap(
        plot.score.dt[
            JMJD6.status == JMJD6 &
            K_count %in% k.count.val,
        ...
        border = "black",
        column_title = paste0(JMJD6.status, ": k ratio (", k.count.val, "/6)")
    )
    return(hm)
}
...
for(jmjd6.status in c("WT", "JMJD6KO")){
    for(k.count in 1:5){
        hm <- plotHeatmap(k.count, JMJD6.status = jmjd6.status, all.score.dt)
        draw(hm)
    }
}
